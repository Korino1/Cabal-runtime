...
Cabal Runtime - Rust-проект, который переносит детерминированную техническую обработку за пределы рабочего контекста модели в Codex. Целевое поведение: модель получает только минимальный семантический результат, а не сырые логи сборки и тестов, полные Git diff, хеши, пути артефактов и служебную оркестрацию.

Сейчас реализованы независимые нормализаторы build/test-логов и Git diff, а также native plugin slice для поддерживаемых Bash-результатов. Этот slice заменяет перехваченный сырой вывод компактной проекцией и при ошибках работает fail-closed. Полная незаметность для всех встроенных путей Codex, включая WebSearch и неперехватываемые shell-вызовы, пока не реализована: для неё требуется Core-side граница до записи результата инструмента в историю модели. Поэтому текущая версия является исходным checkpoint, а не release-complete прозрачным runtime.
